...
<template>
    <ActionSection>
        ...
        <template #description>
            {{ $t('profile.two_factor_authentication_description') }}
        </template>
        ...
        <template #content>
            ...
            <h3 v-else-if="twoFactorEnabled && confirming" class="text-lg font-medium text-gray-900 dark:text-gray-100">
                {{ $t('profile.two_factor_authentication_finish') }}
            </h3>

            <h3 v-else class="text-lg font-medium text-gray-900 dark:text-gray-100">
                {{ $t('profile.two_factor_authentication_not_enabled') }}
            </h3>

            <div class="mt-3 max-w-xl text-sm text-gray-600 dark:text-gray-400">
                <p>
                    {{ $t('profile.two_factor_authentication_authenticator') }}
                </p>
            </div>
            ...
            <div v-if="twoFactorEnabled">
                <div v-if="qrCode">
                    <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-gray-400">
                        <p v-if="confirming" class="font-semibold">
                            {{ $t('profile.two_factor_authentication_authenticator_confirming') }}
                        </p>

                        <p v-else>
                            {{ $t('profile.two_factor_authentication_authenticator_enabled') }}
                        </p>
                    </div>
                ...
                <div v-if="recoveryCodes.length > 0 && ! confirming">
                    <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-gray-400">
                        <p class="font-semibold">
                            {{ $t('profile.two_factor_authentication_authenticator_enabled_store') }}
                        </p>
                    </div>
                ...
                <div v-else>
                    ...
                    <ConfirmsPassword @confirmed="regenerateRecoveryCodes">
                        <SecondaryButton
                            ...
                            class="me-3"
                        >
                            {{ $t('profile.two_factor_authentication_regenerate') }}
                        </SecondaryButton>
                    </ConfirmsPassword>
                    ...
                    <ConfirmsPassword @confirmed="showRecoveryCodes">
                        <SecondaryButton
                            ...
                            class="me-3"
                        >
                            {{ $t('profile.two_factor_authentication_show_recovery_codes') }}
                        </SecondaryButton>
                    </ConfirmsPassword>
